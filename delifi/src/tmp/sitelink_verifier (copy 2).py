        ...
        for id in list(results["query"]["pages"]):
            try:
                ...
                try:
                    ...
                    #self.find_false_positives(results["query"]["pages"][id], qid):

                    try:
                        print('Wikipedia article doesn\'t have a Wikidata link', results["query"]["pages"][id], qid)
                    except UnboundLocalError:
                        print('Wikipedia article doesn\'t have a Wikidata link', results["query"]["pages"][id], 'unknown QID')
